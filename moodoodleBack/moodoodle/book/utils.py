...
Analyze the user's {diary} to understand the user's emotions and recommend books
...
You should not recommend books that are not in the dataframe.
...
def recommend_book(diary):
    ...
    prompt = PromptTemplate(template=system_template, input_variables=["diary", "recommend_format"])

    chain = (
            prompt |
            agent
    )
    result = chain.invoke({"diary":diary, "recommend_format": recommend_format})
    result = result['output'].split('|')
    if len(result) == 4:
        return result
    return None

print(recommend_book('오늘은 매우 기쁘다!'))